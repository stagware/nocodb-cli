...
function startServer() {
  const server = http.createServer((req, res) => {
    ...
    if (req.url?.startsWith("/api/v2/items/")) {
      ...
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: true, itemId, q: url.searchParams.get("q") }));
      return;
    }
...
describe("dynamic api command registration", () => {
  afterEach(() => {
  ...
  it("loads swagger, registers api commands, and executes generated command", async () => {
    ...
    try {
      const configDir = fs.mkdtempSync(path.join(os.tmpdir(), "nocodb-cli-api-"));
      process.env.NOCO_QUIET = "1";
      await runCli(["config", "set", "baseUrl", baseUrl], configDir);

      process.env.NOCO_QUIET = "0";
      await runCli(
        ["--base", "b1", "api", "items", "get-item", "item-123", "--query", "q=hello", "--pretty"],
        configDir
      );

      const output = logs.find((line) => line.includes("\"ok\""));
      expect(output ?? "").toContain("\"ok\": true");
      expect(output ?? "").toContain("\"itemId\": \"item-123\"");
